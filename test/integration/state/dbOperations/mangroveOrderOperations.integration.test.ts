...
import { AccountId, ChainId, MangroveId, MangroveOrderId, MangroveOrderVersionId, MangroveVersionId, OfferId, OfferListId, OfferListKey, OfferListVersionId, OrderId, TakenOfferId, TokenId } from "../../../../src/state/model";
...
describe("Mangrove Operations Integration test suite", () => {
    ...
    const offerListKey: OfferListKey = { outboundToken: outboundTokenId.tokenAddress, inboundToken: inboundTokenId.tokenAddress };
    ...
    const mangroveOrderId = new MangroveOrderId({ mangroveId: mangroveId, offerListKey: offerListKey, mangroveOrderId: "mangroveOrderId" })
    const mangroveOrderVersionId = new MangroveOrderVersionId({ mangroveOrderId: mangroveOrderId, versionNumber: 0 })
    ...
    beforeEach(async () => {
        ...
        await prisma.offerListVersion.create({data: {
    ...
    describe("addMangroveOrderVersionFromOfferId", () => {
        it("updates to cancelled", async () => {
            ...
            const oldMangroveOrder = await prisma.mangroveOrder.findUnique({ where: { id: mangroveOrderId.value } })
            const oldMangroveOrderVersion = await prisma.mangroveOrderVersion.findUnique({ where: { id: oldMangroveOrder?.currentVersionId } })
            await mangroveOrderOperations.addMangroveOrderVersionFromOfferId(offerId, (m) => m.cancelled = true);
            assert.strictEqual(await prisma.mangroveOrder.count(), 1);
            assert.strictEqual(await prisma.mangroveOrderVersion.count(), 2);
            const newMangroveOrder = await prisma.mangroveOrder.findUnique({ where: { id: mangroveOrderId.value } })
            const newMangroveOrderVersion = await prisma.mangroveOrderVersion.findUnique({ where: { id: newMangroveOrder?.currentVersionId } })
            assert.notDeepStrictEqual(oldMangroveOrder, newMangroveOrder);
            const newVersionId = new MangroveOrderVersionId({ mangroveOrderId: mangroveOrderId, versionNumber: 1 })
            assert.strictEqual(oldMangroveOrder?.currentVersionId, mangroveOrderVersionId.value);
            assert.strictEqual(newMangroveOrder?.currentVersionId, newVersionId.value);
            assert.notDeepStrictEqual(oldMangroveOrderVersion, newMangroveOrderVersion);
    ...
    describe("getCurrentMangroveOrderVersion", () => {
        ...
        it("No MangroveOrder, with mangroveOrder", async () => {
            ...
            await assert.rejects(mangroveOrderOperations.getCurrentMangroveOrderVersion({mangroveOrder}))
        })
        ...
        it("Has current version, with mangroveOrder", async () => {
            const currentVersion = await mangroveOrderOperations.getCurrentMangroveOrderVersion({mangroveOrder});
        ...
        it("No MangroveOrder, with mangroveOrderId", async () => {
            const noMatch = new MangroveOrderId({ mangroveId:mangroveId, offerListKey: offerListKey, mangroveOrderId: "noMatch"})
            await assert.rejects(mangroveOrderOperations.getCurrentMangroveOrderVersion({mangroveOrderId:noMatch}))
        })

        it("No current version, with mangroveOrderId", async () => {
            await prisma.mangroveOrderVersion.deleteMany();
            await assert.rejects(mangroveOrderOperations.getCurrentMangroveOrderVersion({mangroveOrderId}))
        })

        it("Has current version, with mangroveOrderId", async () => {
            const currentVersion = await mangroveOrderOperations.getCurrentMangroveOrderVersion({mangroveOrderId});
            assert.strictEqual(currentVersion.id, mangroveOrder.currentVersionId)
            assert.strictEqual(currentVersion.mangroveOrderId, mangroveOrderId.value)
        })
    ...
    describe("addMangroveOrderVersion", () => {
        it("MangroveOrder.id != id", async () => {
            const manOrderId = new MangroveOrderId({ mangroveId: mangroveId, offerListKey: offerListKey, mangroveOrderId: "noMatch" });
            await assert.rejects(mangroveOrderOperations.addMangroveOrderVersion(manOrderId, mangroveOrder, mangroveOrderVersion))
        })
        ...
        it("MangroveOrderVersion does not exist", async () => {
            await prisma.mangroveOrderVersion.deleteMany();
            await assert.rejects(mangroveOrderOperations.addMangroveOrderVersion(mangroveOrderId, mangroveOrder, mangroveOrderVersion))
        })
        ...
        it("MangroveOrderVersion does exist", async () => {
            ...
            const oldMangroveOrder = await prisma.mangroveOrder.findUnique({ where: { id: mangroveOrderId.value } })
            const oldMangroveOrderVersion = await prisma.mangroveOrderVersion.findUnique({ where: { id: oldMangroveOrder?.currentVersionId } })
            await mangroveOrderOperations.addMangroveOrderVersion(mangroveOrderId, mangroveOrder, mangroveOrderVersion);
            assert.strictEqual(await prisma.mangroveOrder.count(), 1);
            assert.strictEqual(await prisma.mangroveOrderVersion.count(), 2);
            const newMangroveOrder = await prisma.mangroveOrder.findUnique({ where: { id: mangroveOrderId.value } })
            const newMangroveOrderVersion = await prisma.mangroveOrderVersion.findUnique({ where: { id: newMangroveOrder?.currentVersionId } })
    ...
    describe("deleteLatestMangroveOrderVersionUsingOfferId", () => {
        it("deletes", async () => {
            assert.strictEqual(await prisma.mangroveOrderVersion.count(), 1);
            await mangroveOrderOperations.deleteLatestMangroveOrderVersionUsingOfferId(offerId);
    ...
    describe("deleteLatestVersionOfMangroveOrder", () => {
        it("MangroveOrder does not exist", async () => {
            const mangroveOrderId2 = new MangroveOrderId({ mangroveId: mangroveId, offerListKey: offerListKey, mangroveOrderId: "noMatch" })
            await assert.rejects(mangroveOrderOperations.deleteLatestVersionOfMangroveOrder(mangroveOrderId2));
        })
        ...
        it("MangroveOrderVersion deleted", async () => {
            ...
            assert.strictEqual(await prisma.mangroveOrderVersion.count(), 2);
            const oldMangroveOrder = await prisma.mangroveOrder.findUnique({ where: { id: mangroveOrderId.value } })
            const deletedMangroveOrderVersion = await prisma.mangroveOrderVersion.findUnique({ where: { id: oldMangroveOrder?.currentVersionId } })
            await mangroveOrderOperations.deleteLatestVersionOfMangroveOrder(mangroveOrderId);
            assert.strictEqual(await prisma.mangroveOrder.count(), 1);
            assert.strictEqual(await prisma.mangroveOrderVersion.count(), 1);
            const newMangroveOrder = await prisma.mangroveOrder.findUnique({ where: { id: mangroveOrderId.value } })
            const newMangroveOrderVersion = await prisma.mangroveOrderVersion.findUnique({ where: { id: newMangroveOrder?.currentVersionId } })
            assert.notDeepStrictEqual(oldMangroveOrder, newMangroveOrder);
            const deletedVersionId = new MangroveOrderVersionId({ mangroveOrderId: mangroveOrderId, versionNumber: 1 })
            assert.strictEqual(oldMangroveOrder?.currentVersionId, deletedVersionId.value);
            assert.strictEqual(newMangroveOrder?.currentVersionId, mangroveOrderVersionId.value);
            assert.notDeepStrictEqual(deletedMangroveOrderVersion, newMangroveOrderVersion);
    ...
    describe("createMangroveOrder", () => {
        it("creates mangroveOrder", async () => {
            ...
            const newMangroveOrder = await prisma.mangroveOrder.findUnique({where: {id: mangroveOrder.id}});
            if(!newMangroveOrder){
                assert.fail()
            }
    ...
    describe("createMangroveOrderVersion", () => {
        it("creates mangroveOrderVersion", async () => {
            ...
            assert.strictEqual(await prisma.mangroveOrderVersion.count(), 1);
            await mangroveOrderOperations.createMangroveOrderVersion(mangroveOrderVersion);
            assert.strictEqual(await prisma.mangroveOrderVersion.count(), 2);
            const newMangroveOrderVersion = await prisma.mangroveOrderVersion.findUnique({where: {id: mangroveOrderVersion.id}});
