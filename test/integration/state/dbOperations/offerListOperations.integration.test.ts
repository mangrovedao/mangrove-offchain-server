...
import { ChainId, MangroveId, MangroveVersionId, OfferId, OfferListId, OfferListKey, OfferListVersionId, TokenId } from "../../../../src/state/model";
...
describe("OfferList Operations Integration test suite", () => {
    ...
    describe("getOfferListTokens", () => {
        it("No offerList with that Id, offerListId", async () => {
            ...
            await assert.rejects( offerListOperations.getOfferListTokens({id:noMatch}))
        })

        it("get offerList, offerListId", async () => {
            const tokens = await offerListOperations.getOfferListTokens({id:offerListId});
        ...
        it("No offerList with that Id, mangroveOrder", async () => {
            offerListKey.inboundToken = "noMatch"
            const noMatch = new OfferListId(mangroveId, offerListKey);
            await assert.rejects( offerListOperations.getOfferListTokens({mangroveOrder:{ offerListId:noMatch.value}}))
        })

        it("get offerList, mangroveOrder", async () => {
            const tokens = await offerListOperations.getOfferListTokens({mangroveOrder: {offerListId: offerListId.value}});
            assert.deepStrictEqual(tokens.inboundToken, inboundToken);
            assert.deepStrictEqual(tokens.outboundToken, outboundToken);
        })
    })
    ...
    describe("addVersionedOfferList", () => {
        it("Create new offerList and version", async () => {
            ...
            await offerListOperations.addVersionedOfferList( newOfferListId, "txId", (o) => o.active=true);
            assert.strictEqual(await prisma.offerList.count(), 2);
            assert.strictEqual(await prisma.offerListVersion.count(), 2);
            const newOfferList = await prisma.offerList.findUnique({where: { id: newOfferListId.value}});
            ...
            const newOfferListVersion = await prisma.offerListVersion.findUnique({where: { id: newOfferListVersionId.value}});  
        ...
        it("Updates offerList and adds new version", async () => {
            assert.strictEqual(await prisma.offerList.count(), 1);
            assert.strictEqual(await prisma.offerListVersion.count(), 1);
            await offerListOperations.addVersionedOfferList( offerListId, "txId", (o) => o.active=false);
            assert.strictEqual(await prisma.offerList.count(), 1);
            assert.strictEqual(await prisma.offerListVersion.count(), 2);
            const newOfferList = await prisma.offerList.findUnique({where: { id: offerListId.value}});
            assert.notDeepStrictEqual(newOfferList, null);
        ...
        it("Cant find current version", async () => {
            ...
            await assert.rejects( offerListOperations.addVersionedOfferList( offerListId, "txId", (o) => o.active=false) );
            assert.strictEqual(await prisma.offerList.count(), 1);
            assert.strictEqual(await prisma.offerListVersion.count(), 0);
        })
    })
    ...
    describe("deleteLatestOfferListVersion", () => {
        it("cant find offerList", async () => {
            offerListKey.inboundToken = "noMatch";
            const noMatch = new OfferListId(mangroveId, offerListKey);
            await assert.rejects( offerListOperations.deleteLatestOfferListVersion(noMatch) );
        })
        ...
        it("Has prevVersion, delete only latest version", async () => {
            ...
            assert.strictEqual(await prisma.offerList.count(), 1);
            assert.strictEqual(await prisma.offerListVersion.count(), 2);
            const oldVersion = await prisma.offerList.findUnique({where: { id: offerList.id}});
            await offerListOperations.deleteLatestOfferListVersion( offerListId );
            assert.strictEqual(await prisma.offerList.count(), 1);
            assert.strictEqual(await prisma.offerListVersion.count(), 1);
            const currentVersion = await prisma.offerList.findUnique({where: { id: offerList.id}});
