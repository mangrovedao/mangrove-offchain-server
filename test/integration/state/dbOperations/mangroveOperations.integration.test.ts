...
describe("Mangrove Operations Integration test suite", () => {
    ...
    describe("createMangrove", () => {
        it("Already exists", async () => {
            ...
            await assert.rejects(mangroveOperations.createMangrove(mangroveId, chainId, "address", "txId"));
        })
        ...
        it("Does not exist, will be created", async () => {
            ...
            await mangroveOperations.createMangrove(mangroveId, chainId, "address", "txId");
            assert.strictEqual(await prisma.mangrove.count(), 2);
            assert.strictEqual(await prisma.mangroveVersion.count(), 2);
            const mangrove = await prisma.mangrove.findUnique({ where: { id: mangroveId.value } });
            ...
            const mangroveVersion = await prisma.mangroveVersion.findUnique({ where: { id: mangrove.currentVersionId } });
    ...
    describe("addVersionedMangrove", () => {
        it( "Mangrove does not exist", async () => {
            const chainId = new ChainId(10);
            const mangroveId = new MangroveId(chainId, "mangroveId2");
            await assert.rejects( mangroveOperations.addVersionedMangrove(mangroveId, (m) => m.dead = true , "txId" ) );
        })
        ...
        it("Updates Mangrove and Mangrove version", async () => {
            ...
            assert.strictEqual(await prisma.mangrove.count(), 1);
            assert.strictEqual(await prisma.mangroveVersion.count(), 1);
            const oldMangrove = await prisma.mangrove.findUnique({where: {id: mangroveId.value}})
            const oldMangroveVersion = await prisma.mangroveVersion.findUnique({where: {id: oldMangrove?.currentVersionId}})
            await mangroveOperations.addVersionedMangrove(mangroveId, (m) => m.dead = true , "txId" );
            assert.strictEqual(await prisma.mangrove.count(), 1);
            assert.strictEqual(await prisma.mangroveVersion.count(), 2);
            const newMangrove = await prisma.mangrove.findUnique({where: {id: mangroveId.value}})
            const newMangroveVersion = await prisma.mangroveVersion.findUnique({where: {id: newMangrove?.currentVersionId}})
    ...
    describe("deleteLatestMangroveVersion", () => {
        it("Mangrove does not exist", async () => {
            const chainId = new ChainId(10);
            const mangroveId = new MangroveId(chainId, "mangroveId2");
            await assert.rejects(mangroveOperations.deleteLatestMangroveVersion(mangroveId));
        })
